...
import net.minecraft.client.util.UsefulAccess;
...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    public final UsefulAccess usefulAcces;
    ...
    public Minecraft(GameConfiguration gameConfig)
    {
        ...
        this.usefulAcces = new UsefulAccess(this);
        ...
        Bootstrap.register();
        TextComponentKeybind.getStringSupplier = KeyBinding::func_193626_b;
        this.dataFixer = DataFixesManager.createFixer();
        this.guiToast = new GuiToast(this);
    ...
    public void run()
    {
        ...
        while (true)
        {
            try
            {
                while (this.running)
                {
                    if (!this.hasCrashed || this.crashReporter == null)
                    {
                        ...
                        catch (OutOfMemoryError outOfMemoryE)
                        {
                            this.freeMemory();
            ...
            catch (MinecraftError minecraftE)
            {
                break;
            }
            catch (ReportedException reportedE)
            {
                this.addGraphicsAndWorldToCrashReport(reportedE.getCrashReport());
                this.freeMemory();
                LOGGER.fatal("Reported exception thrown!", (Throwable)reportedE);
                this.displayCrashReport(reportedE.getCrashReport());
                break;
            }
    ...
    private void func_193986_ar()
    {
        ...
        this.searchTreeManager.func_194009_a(SearchTreeManager.field_194012_b, searchtree1);
    }
    ...
    private void runGameLoop() throws IOException
    {
        ...
        if (!this.skipRenderWorld)
        {
            ...
            this.guiToast.showToast(new ScaledResolution(this));
            this.mcProfiler.endSection();
        }
    ...
    private boolean processKeyF3(int associatedKey /* Key number, for F3 + combinations. See end of file for mapping*/)
    {
        if (associatedKey == 30) // F3 + A : Reloads all chunks
        {
            this.renderGlobal.loadRenderers();
            this.func_190521_a("debug.reload_chunks.message");
            return true;
        }
        else if (associatedKey == 48) // F3 + B : Show hitboxes
        {
            ...
            return true;
        } else if (associatedKey == 31) // F3 + S : Clear the sent messages
        {
        	this.ingameGUI.getChatGUI().getSentMessages().clear();
        	return true;
        }
        else if (associatedKey == 32) // F3 + D : Clear the chat
        {
        ...
        else if (associatedKey == 33) // F3 + F : Increase (or decrease if shift is pressed) render distance by 1
        {
            this.gameSettings.setOptionValue(GameSettings.Options.RENDER_DISTANCE, GuiScreen.isShiftKeyDown() ? -1 : 1);
            this.func_190521_a("debug.cycle_renderdistance.message", this.gameSettings.renderDistanceChunks);
            return true;
        }
        else if (associatedKey == 34) // F3 + G : Show/Hide chunk doundaries
        {
        ...
        else if (associatedKey == 35) // F3 + H : Activate Advanced tooltips
        {
        ...
        else if (associatedKey == 49) // F3 + N : Switch between creative and spectator gamemode
        {
            if (!this.player.canCommandSenderUseCommand(2, "gamemode"))
            {
        ...
        else if (associatedKey == 25) // F3 + P : Switch the "Pause on lost focus" feature
        {
        ...
        else if (associatedKey == 16) // F3 + Q : Display F3 debug keys help
        {
        ...
        else if (associatedKey == 20) // F3 + T : Reloads ressource packs
        {
    ...
    public void loadWorld(@Nullable WorldClient worldClientIn, String loadingMessage)
    {
        if (worldClientIn == null)
        {
            ...
            NarratorChatListener.narratorChatListener.clear();
        }
