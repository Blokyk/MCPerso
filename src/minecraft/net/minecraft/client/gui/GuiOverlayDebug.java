...
public class GuiOverlayDebug extends Gui
{
    ...
    public GuiOverlayDebug(Minecraft mcInstance)
    {
        this.mcInstance = mcInstance;
        this.fontRenderer = mcInstance.fontRendererObj;
    }

    public void renderDebugInfo(ScaledResolution scaledResolutionIn)
    {
        this.mcInstance.mcProfiler.startSection("debug");
        GlStateManager.pushMatrix();
        this.renderDebugInfoLeft();
        this.renderDebugInfoRight(scaledResolutionIn);
        GlStateManager.popMatrix();

        if (this.mcInstance.gameSettings.showLagometer)
        {
            this.renderLagometer();
        }

        this.mcInstance.mcProfiler.endSection();
    }
    ...
    protected void renderDebugInfoLeft()
    {
        ...
        list.add("");
        list.add("Debug: Pie [shift]: " + (this.mcInstance.gameSettings.showDebugProfilerChart ? "visible" : "hidden") + " FPS [alt]: " + (this.mcInstance.gameSettings.showLagometer ? "visible" : "hidden"));
    ...
    {
        BlockPos blockpos = new BlockPos(this.mcInstance.getRenderViewEntity().posX, this.mcInstance.getRenderViewEntity().getEntityBoundingBox().minY, this.mcInstance.getRenderViewEntity().posZ);

        if (this.mcInstance.isReducedDebug())
        {
            return Lists.newArrayList("Minecraft 1.12 (" + this.mcInstance.getVersion() + "/" + ClientBrandRetriever.getClientModName() + ")", this.mcInstance.debug, this.mcInstance.renderGlobal.getDebugInfoRenders(), this.mcInstance.renderGlobal.getDebugInfoEntities(), "P: " + this.mcInstance.effectRenderer.getStatistics() + ". T: " + this.mcInstance.world.getDebugLoadedEntities(), this.mcInstance.world.getProviderName(), "", String.format("Chunk-relative: %d %d %d", blockpos.getX() & 15, blockpos.getY() & 15, blockpos.getZ() & 15));
        }
        else
        {
            Entity entity = this.mcInstance.getRenderViewEntity();
            ...
            List<String> list = Lists.newArrayList("Minecraft 1.12 (" + this.mcInstance.getVersion() + "/" + ClientBrandRetriever.getClientModName() + ("release".equalsIgnoreCase(this.mcInstance.getVersionType()) ? "" : "/" + this.mcInstance.getVersionType()) + ")", this.mcInstance.debug, this.mcInstance.renderGlobal.getDebugInfoRenders(), this.mcInstance.renderGlobal.getDebugInfoEntities(), "P: " + this.mcInstance.effectRenderer.getStatistics() + ". T: " + this.mcInstance.world.getDebugLoadedEntities(), this.mcInstance.world.getProviderName(), "", String.format("XYZ: %.3f / %.5f / %.3f", this.mcInstance.getRenderViewEntity().posX, this.mcInstance.getRenderViewEntity().getEntityBoundingBox().minY, this.mcInstance.getRenderViewEntity().posZ), String.format("Block: %d %d %d", blockpos.getX(), blockpos.getY(), blockpos.getZ()), String.format("Chunk: %d %d %d in %d %d %d", blockpos.getX() & 15, blockpos.getY() & 15, blockpos.getZ() & 15, blockpos.getX() >> 4, blockpos.getY() >> 4, blockpos.getZ() >> 4), String.format("Facing: %s (%s) (%.1f / %.1f)", enumfacing, s, MathHelper.wrapDegrees(entity.rotationYaw), MathHelper.wrapDegrees(entity.rotationPitch)));

            if (this.mcInstance.world != null)
            {
                Chunk chunk = this.mcInstance.world.getChunkFromBlockCoords(blockpos);

                if (this.mcInstance.world.isBlockLoaded(blockpos) && blockpos.getY() >= 0 && blockpos.getY() < 256)
                {
                    if (!chunk.isEmpty())
                    {
                        list.add("Biome: " + chunk.getBiome(blockpos, this.mcInstance.world.getBiomeProvider()).getBiomeName());
                        ...
                        DifficultyInstance difficultyinstance = this.mcInstance.world.getDifficultyForLocation(blockpos);

                        if (this.mcInstance.isIntegratedServerRunning() && this.mcInstance.getIntegratedServer() != null)
                        {
                            EntityPlayerMP entityplayermp = this.mcInstance.getIntegratedServer().getPlayerList().getPlayerByUUID(this.mcInstance.player.getUniqueID());
                        ...
                        list.add(String.format("Local Difficulty: %.2f // %.2f (Day %d)", difficultyinstance.getAdditionalDifficulty(), difficultyinstance.getClampedAdditionalDifficulty(), this.mcInstance.world.getWorldTime() / 24000L));
                    }
                    else
                    {
            ...
            if (this.mcInstance.entityRenderer != null && this.mcInstance.entityRenderer.isShaderActive())
            {
                list.add("Shader: " + this.mcInstance.entityRenderer.getShaderGroup().getShaderGroupName());
            }

            if (this.mcInstance.objectMouseOver != null && this.mcInstance.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK && this.mcInstance.objectMouseOver.getBlockPos() != null)
            {
                BlockPos blockpos1 = this.mcInstance.objectMouseOver.getBlockPos();
    ...
    protected <T extends Comparable<T>> List<String> getDebugInfoRight()
    {
        ...
        long l = j - k;
        List<String> list = Lists.newArrayList(String.format("Java: %s %dbit", System.getProperty("java.version"), this.mcInstance.isJava64bit() ? 64 : 32), String.format("Mem: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i)), String.format("Allocated: % 2d%% %03dMB", j * 100L / i, bytesToMb(j)), "", String.format("CPU: %s", OpenGlHelper.getCpu()), "", String.format("Display: %dx%d (%s)", Display.getWidth(), Display.getHeight(), GlStateManager.glGetString(7936)), GlStateManager.glGetString(7937), GlStateManager.glGetString(7938));
        ...
        else
        {
            if (this.mcInstance.objectMouseOver != null && this.mcInstance.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK && this.mcInstance.objectMouseOver.getBlockPos() != null)
            {
                BlockPos blockpos = this.mcInstance.objectMouseOver.getBlockPos();
                IBlockState iblockstate = this.mcInstance.world.getBlockState(blockpos);

                if (this.mcInstance.world.getWorldType() != WorldType.DEBUG_WORLD)
                {
                    iblockstate = iblockstate.getActualState(this.mcInstance.world, blockpos);
                }
    ...
    private void renderLagometer()
    {
        ...
        FrameTimer frametimer = this.mcInstance.getFrameTimer();
        ...
        if (this.mcInstance.gameSettings.limitFramerate <= 120)
        {
            this.drawHorizontalLine(0, 239, scaledresolution.getScaledHeight() - 60 + this.mcInstance.gameSettings.limitFramerate / 2, -16711681);
        }
