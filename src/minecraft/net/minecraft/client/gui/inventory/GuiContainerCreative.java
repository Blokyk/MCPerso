...
public class GuiContainerCreative extends InventoryEffectRenderer
{
    ...
    protected void handleMouseClick(@Nullable Slot slotIn, int slotId, int mouseButton, ClickType type)
    {
        ...
        if (slotIn == null && selectedTabIndex != CreativeTabs.INVENTORY.getTabIndex() && type != ClickType.QUICK_CRAFT)
        {
            ...
            if (!inventoryplayer1.getItemStack().func_190926_b())
            {
                if (mouseButton == 0)
                {
                    ...
                    inventoryplayer1.setItemStack(ItemStack.nullItemStack);
                }
        ...
        else
        {
            ...
            if (slotIn == this.destroyItemSlot && flag)
            {
                ...
                {
                    this.mc.playerController.sendSlotPacket(ItemStack.nullItemStack, j);
                }
            }
            else if (selectedTabIndex == CreativeTabs.INVENTORY.getTabIndex())
            {
                ...
                {
                    this.mc.player.inventory.setItemStack(ItemStack.nullItemStack);
                }
            ...
            {
                ItemStack itemstack3 = slotIn == null ? ItemStack.nullItemStack : this.inventorySlots.getSlot(slotIn.slotNumber).getStack();
    ...
    private void setCurrentCreativeTab(CreativeTabs tab)
    {
        ...
        if (tab == CreativeTabs.field_192395_m)
        {
            for (int j = 0; j < 9; ++j)
            {
                HotbarSnapshot hotbarsnapshot = this.mc.creativeSettings.getHotbarAt(j);
                ...
                if (hotbarsnapshot.isEmpty())
                {
                    for (int k = 0; k < 9; ++k)
                    {
                        ...
                        {
                            guicontainercreative$containercreative.itemList.add(ItemStack.nullItemStack);
                        }
                    }
                }
                else
    ...
    public static void func_192044_a(Minecraft mcInstance, int hotbarIndex, boolean p_192044_2_, boolean p_192044_3_)
    {
        EntityPlayerSP entityplayersp = mcInstance.player;
        CreativeSettings creativesettings = mcInstance.creativeSettings;
        HotbarSnapshot hotbarsnapshot = creativesettings.getHotbarAt(hotbarIndex);
        ...
        if (p_192044_2_)
        {
            for (int i = 0; i < InventoryPlayer.getHotbarSize(); ++i)
            {
                ...
                mcInstance.playerController.sendSlotPacket(itemstack, 36 + i);
            }
        ...
        else if (p_192044_3_)
        {
            ...
            String s = GameSettings.getKeyDisplayString(mcInstance.gameSettings.keyBindsHotbar[hotbarIndex].getKeyCode());
            String s1 = GameSettings.getKeyDisplayString(mcInstance.gameSettings.keyBindLoadToolbar.getKeyCode());
            mcInstance.ingameGUI.setRecordPlaying(new TextComponentTranslation("inventory.hotbarSaved", new Object[] {s1, s}), false);
        ...
        public void scrollTo(float p_148329_1_)
        {
            ...
            for (int k = 0; k < 5; ++k)
            {
                for (int l = 0; l < 9; ++l)
                {
                    ...
                    {
                        GuiContainerCreative.basicInventory.setInventorySlotContents(l + k * 9, ItemStack.nullItemStack);
                    }
                }
            }
        }
