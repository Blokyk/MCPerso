...
public abstract class GuiContainer extends GuiScreen
{
    ...
    /** Used when touchscreen is enabled */
    private ItemStack draggedStack = ItemStack.nullItemStack;
    ...
    /** Used when touchscreen is enabled */
    private ItemStack returningStack = ItemStack.nullItemStack;
    ...
    private ItemStack shiftClickedSlot = ItemStack.nullItemStack;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        ItemStack itemstack = this.draggedStack.isNull() ? inventoryplayer.getItemStack() : this.draggedStack;

        if (!itemstack.isNull())
        {
            int j2 = 8;
            int k2 = this.draggedStack.isNull() ? 8 : 16;
            String s = null;

            if (!this.draggedStack.isNull() && this.isRightMouseClick)
            {
        ...
        if (!this.returningStack.isNull())
        {
            ...
            {
                f = 1.0F;
                this.returningStack = ItemStack.nullItemStack;
            }
    ...
    {
        if (this.mc.player.inventory.getItemStack().isNull() && this.theSlot != null && this.theSlot.getHasStack())
        {
    ...
    private void drawItemStack(ItemStack stack, int x, int y, String altText)
    {
        ...
        this.itemRender.renderItemOverlayIntoGUI(this.fontRendererObj, stack, x, y - (this.draggedStack.isNull() ? 0 : 8), altText);
        this.zLevel = 0.0F;
    ...
    private void drawSlot(Slot slotIn)
    {
        ...
        boolean flag = false;
        boolean flag1 = slotIn == this.clickedSlot && !this.draggedStack.isNull() && !this.isRightMouseClick;
        ItemStack itemstack1 = this.mc.player.inventory.getItemStack();
        String s = null;

        if (slotIn == this.clickedSlot && !this.draggedStack.isNull() && this.isRightMouseClick && !itemstack.isNull())
        {
            itemstack = itemstack.copy();
            itemstack.func_190920_e(itemstack.func_190916_E() / 2);
        }
        else if (this.dragSplitting && this.dragSplittingSlots.contains(slotIn) && !itemstack1.isNull())
        {
            ...
            if (Container.canAddItemToSlot(slotIn, itemstack1, true) && this.inventorySlots.canDragIntoSlot(slotIn))
            {
                ...
                flag = true;
                Container.computeStackSize(this.dragSplittingSlots, this.dragSplittingLimit, itemstack, slotIn.getStack().isNull() ? 0 : slotIn.getStack().func_190916_E());
        ...
        if (itemstack.isNull() && slotIn.canBeHovered())
        {
    ...
    private void updateDragSplitting()
    {
        ...
        if (!itemstack.isNull() && this.dragSplitting)
        {
            ...
            else
            {
                ...
                for (Slot slot : this.dragSplittingSlots)
                {
                    ...
                    int i = itemstack2.isNull() ? 0 : itemstack2.func_190916_E();
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        if (mouseButton == 0 || mouseButton == 1 || flag)
        {
            ...
            if (this.mc.gameSettings.touchscreen && flag1 && this.mc.player.inventory.getItemStack().isNull())
            {
            ...
            if (l != -1)
            {
                if (this.mc.gameSettings.touchscreen)
                {
                    if (slot != null && slot.getHasStack())
                    {
                        ...
                        this.draggedStack = ItemStack.nullItemStack;
                ...
                {
                    if (this.mc.player.inventory.getItemStack().isNull())
                    {
                        ...
                        else
                        {
                            ...
                            if (flag2)
                            {
                                this.shiftClickedSlot = slot != null && slot.getHasStack() ? slot.getStack().copy() : ItemStack.nullItemStack;
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick)
    {
        ...
        if (this.clickedSlot != null && this.mc.gameSettings.touchscreen)
        {
            ...
            {
                if (this.draggedStack.isNull())
                {
                    if (slot != this.clickedSlot && !this.clickedSlot.getStack().isNull())
                    {
        ...
        else if (this.dragSplitting && slot != null && !itemstack.isNull() && (itemstack.func_190916_E() > this.dragSplittingSlots.size() || this.dragSplittingLimit == 2) && Container.canAddItemToSlot(slot, itemstack, true) && slot.isItemValid(itemstack) && this.inventorySlots.canDragIntoSlot(slot))
        {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state)
    {
        ...
        if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot(ItemStack.nullItemStack, slot))
        {
            if (isShiftKeyDown())
            {
                if (!this.shiftClickedSlot.isNull())
                {
        ...
        else
        {
            ...
            if (this.clickedSlot != null && this.mc.gameSettings.touchscreen)
            {
                if (state == 0 || state == 1)
                {
                    if (this.draggedStack.isNull() && slot != this.clickedSlot)
                    {
                    ...
                    if (k != -1 && !this.draggedStack.isNull() && flag2)
                    {
                    ...
                    else if (!this.draggedStack.isNull())
                    {
                        this.touchUpX = mouseX - i;
                        this.touchUpY = mouseY - j;
                        this.returningStackDestSlot = this.clickedSlot;
            ...
            else if (!this.mc.player.inventory.getItemStack().isNull())
            {
    ...
    {
        if (this.mc.player.inventory.getItemStack().isNull() && this.theSlot != null)
        {
            for (int i = 0; i < 9; ++i)
            {
